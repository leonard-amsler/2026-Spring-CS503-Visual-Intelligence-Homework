...
class VQACollator(object):  # Visual Question Answering Collator
    ...
    def __call__(self, batch):
        ...
        input_ids = encoded_full_sequences['input_ids'] # shape → (batch_size, seq_len)
        ...
        labels = torch.roll(labels, shifts=-1, dims=1)  # in a causal LM, the target at position t is the token at position t+1
        ...
        original_lengths = [len(self.tokenizer.encode(seq, add_special_tokens=False)) for seq in input_sequences]
        # Then iterate over each sample i of the batch and handle two cases:
        for i in range(len(batch)):
            # Get the length of the question for this sample
            question_length = len(self.tokenizer.encode(texts[i], add_special_tokens=False)) # the question length is the number of tokens in the question (without special tokens)
            ...
            if original_lengths[i] > self.max_length:
                labels[i] = -100 # set all labels to -100 to ignore this sample entirely
