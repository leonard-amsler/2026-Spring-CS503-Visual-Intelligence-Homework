...
class LanguageModel(nn.Module):
    ...
    def forward_kv(self, x, past_key_values=None, attention_mask=None):
        ...
        past_length = past_key_values[0][0].size(2) if past_key_values is not None else 0 # past length is 0 if past_key_values is None, otherwise get the length from past_key_values
        ...
        for i, block in enumerate(self.blocks):
            past_kv = past_key_values[i] if past_key_values is not None else None  # fetch this layer's cache entry (or None on first call)
